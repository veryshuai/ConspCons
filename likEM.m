...
display(['We have a total of ', num2str(size(c.exp,1)), ' usable observations']);
...
%scale
%scl = abs(likEM_loop(init,cons,exp,y,price,ot_ind,w_b,c_b,1));
scl = 1;
init = .5;
...
options=optimset('Display','iter','Algorithm','interior-point','TolCon',1e-3,'TolX',1e-6,'TolFun',1e-3,'MaxFunEvals',1e5,'MaxIter',5,'SubProblemAlgorithm','cg');%,'Hessian','bfgs');

[X,fval,exitflag] = ktrlink(@(x) likEM_loop(x,cons,exp,y,price,ot_ind,w_b,c_b,scl),init,[],[],[],[],...
    0,1,...
    [],options);   
