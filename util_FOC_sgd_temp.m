function [u,foc] = util_FOC(p,cf,w,price,scl,gcf,alp,s,v)
...
for j = 1:size(w,2)
    ...
    fu = fu + sum(bsxfun(@times,p(:,1),log(bsxfun(@plus,p(:,2),cns))),1);
    
    diff = bsxfun(@minus,g,cns);
    wp = bsxfun(@times,normpdf(diff,zeros(size(diff)),repmat(v',size(diff,1),1)),ws);
    wp = bsxfun(@rdivide,wp,sum(wp,2));
    ex = sum(prod(wp,2).*su)/size(s,1);
